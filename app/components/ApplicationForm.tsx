...
const formSchema = z.object({
  ...
  teamMembers: z
    ...
    .refine(
      ...
      "Please provide at least 5 words about team members"
    )
    .refine(
      (val) => countWords(val) <= 100,
      "Team members description is too long (max 100 words)"
    ),
  teamExperience: z
    .string()
    .refine(
      (val) => countWords(val) >= 5,
      "Please provide at least 5 words about team experience"
    )
    .refine(
      (val) => countWords(val) <= 100,
      "Team experience description is too long (max 100 words)"
    ),
  ...
  gameConcept: z
    ...
    .refine(
      ...
      "Please provide at least 25 words about your game concept"
    )
    .refine(
      (val) => countWords(val) <= 300,
      "Game concept is too long (max 300 words)"
    ),
  whyWin: z
    .string()
    .refine(
      (val) => countWords(val) >= 15,
      "Please provide at least 15 words about why your game should win"
    )
    .refine(
      (val) => countWords(val) <= 200,
      "Response is too long (max 200 words)"
    ),
  whyPlayersLike: z
    .string()
    .refine(
      (val) => countWords(val) >= 15,
      "Please provide at least 15 words about why players will like your game"
    )
    .refine(
      (val) => countWords(val) <= 200,
      "Response is too long (max 200 words)"
  ...
  promotionPlan: z
    ...
    .refine(
      (val) => countWords(val) >= 15,
      "Please provide at least 15 words about your promotion plan"
    )
    .refine(
      (val) => countWords(val) <= 200,
      "Response is too long (max 200 words)"
  ...
  monetizationPlan: z
    ...
    .refine(
      (val) => countWords(val) >= 15,
      "Please provide at least 15 words about your monetization plan"
    )
    .refine(
      (val) => countWords(val) <= 200,
      "Response is too long (max 200 words)"
  ...
  developmentTimeline: z
    .string()
    .refine(
      (val) => countWords(val) >= 15,
      "Please provide at least 15 words about your development timeline"
    )
    .refine(
      (val) => countWords(val) <= 200,
      "Response is too long (max 200 words)"
  ...
  resourcesTools: z
    ...
    .refine(
      (val) => countWords(val) >= 15,
      "Please provide at least 15 words about required resources and tools"
    )
    .refine(
      (val) => countWords(val) <= 200,
      "Response is too long (max 200 words)"
  ...
  teamExperienceDescription: z
    ...
    .refine(
      (val) => countWords(val) >= 25,
      "Please provide at least 25 words about your team's experience"
    )
    .refine(
      (val) => countWords(val) <= 300,
      "Response is too long (max 300 words)"
    ),
...
const formSections = [
  ...
  {
    title: "Previous Work and Experience",
  ...
  const renderInput = (
    ...
    <div className="space-y-2">
      ...
      <Input
        ...
        } bg-gray-900 text-gray-100 focus:bg-gray-950`}
      />
      {errors[name] && (
  ...
  const renderTextarea = (
    ...
    <div className="space-y-2">
      ...
      <Textarea
        ...
        className={`${
          errors[name] ? "border-red-500" : ""
        } bg-gray-900 text-gray-100 focus:bg-gray-950 min-h-[100px]`}
      />
      {errors[name] && (
        <p className="text-xs text-red-400">{errors[name]?.message}</p>
      )}
        ...
        return (
          <Card className="bg-gray-800 p-6">
            <CardContent className="space-y-4">
              ...
              {renderTextarea(
                "teamExperience",
                "Team's Game Development Experience"
              )}
            </CardContent>
          </Card>
        );
        ...
        return (
          <Card className="bg-gray-800 p-6">
            <CardContent className="space-y-4">
              {renderTextarea(
                "previousProjects",
                "Provide Links to Previous Game Projects"
              )}
              {renderTextarea(
        ...
        {submitStatus && (
          <p
            className={`mt-4 text-center ${
              submitStatus.includes("Error") ? "text-red-400" : "text-green-400"
            }`}
          >
            {submitStatus}
          </p>
